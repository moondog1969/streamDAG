multi.path.visibility <- function(G, degree = "in", source = NULL, sink = NULL, directed = TRUE, weights = NULL){
  G <-  murphy_spring; degree = "in"; source = c("IN_N", "IN_S"); sink = "OUT"; directed = TRUE; weights = NULL
  
  list.paths <- list(1:length(source))
  list.names <- list(1:length(source))
  list.t <- list(1:length(source))
