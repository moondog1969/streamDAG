path.visibility <- function(G, degree = "in", source = NULL, sink = NULL, directed = TRUE){
  ...
  if(n == 2){if(!directed){ vis.alg <- matrix(ncol = 2, c(NA,TRUE,TRUE,NA), dimnames = list(path, path))}
    else{vis.alg <- matrix(ncol = 2, c(NA,FALSE,TRUE,NA), dimnames = list(path, path))
    }
  }
  vis.alg
}
