global.summary <- function(G, sink, inf.paths = FALSE){
  ...
  out <- t(data.frame(Size = size, Diameter = diameter, n.paths.to.sink = Npaths.to.sink, Mean.a.centrality = mean.a.centrality, mean.path.lengths = mean.p.lengths, Randic = randic, first.Zagreb = fst.zagreb, second.Zagreb = scd.zagreb, ABC = ABC, GA = GA, Harm = harm, Assort.in.out = a.in.out, Assort.in.in = a.in.in))
