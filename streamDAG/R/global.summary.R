global.summary <- function(G, which = "all", sink, mode = "in", inf.paths = FALSE){
  if(length(V(G)) == 0) crit <- TRUE else crit <- FALSE
...
#-----------------------------------------------------------# 
    ...
    if(which == "all"| which == "sink.path.len.summary"){ 
      if(crit){path.length.mean <- NaN} else{ 
        path.length.mean <- mean(p.lengths.sink)}}
    
    if(which == "all"| which == "sink.path.len.summary"){ 
      if(crit){path.length.var <- NaN} else{ 
        path.length.var <- var(p.lengths.sink)}}
    
    if(which == "all"| which == "sink.path.len.summary"){ 
      if(crit){path.length.skew <- NaN} else{ 
        path.length.skew <- skew(p.lengths.sink)}}
    
    if(which == "all"| which == "sink.path.len.summary"){ 
      if(crit){path.length.kurt <- NaN} else{ 
